...
def train_lr_transform(crop_size, upscale_factor):
    return Compose(
        [
            ToPILImage(),
            Resize(
                crop_size // (upscale_factor), interpolation=Image.BILINEAR
            ),
...
class ValDatasetFromFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lr_scale = Resize(
            (
                h_crop_size // self.upscale_factor,
                w_crop_size // self.upscale_factor,
            ),
            interpolation=Image.BILINEAR,
        )
        hr_image = Resize(
            (h_crop_size, w_crop_size), interpolation=Image.BICUBIC
        )(hr_image)
